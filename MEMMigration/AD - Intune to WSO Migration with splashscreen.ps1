...
#URLs for the splashscreen zip and enrollmenttracker powershell script
$splashscreen = "https://winsettings.blob.core.windows.net/winsettings/Splashscreen.zip?sp=r&st=202"
$EnrollmentTracker = "https://winsettings.blob.core.windows.net/winsettings/CheckEnrollment.ps1?sp=r&st=202"
...
$output = "C:\Temp\splashscreen.zip"
$wc = New-Object System.Net.WebClient
$wc.DownloadFile($splashscreen, $output)

#Remove if you don't ZIP the PPKG
Expand-Archive -Path "C:\Temp\splashscreen.zip" -DestinationPath C:\Temp\Splashscreen -Force

#Download enrollmenttracker
$output = "C:\Temp\CheckEnrollment.ps1"
$wc = New-Object System.Net.WebClient
$wc.DownloadFile($EnrollmentTracker, $output)
...
{
   $Check = (Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Enrollments\Status\$($ID)").LifecycleNotificationStatus
   if($Check -eq 1){   $MDMID = $ID }
}
$MDMAuthority = (Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Enrollments\$($MDMID)").ProviderID
...
#Start splashscreen
$args = '-NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -command "C:\Temp\Splashscreen\Show-OSUpgradeBackground.ps1"'
Start-Process C:\windows\system32\WindowsPowerShell\v1.0\powershell.exe -ArgumentList $args

#Start Enrollment tracker
$args = '-NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -command "C:\Temp\CheckEnrollment.ps1 -APIEndpoint $($APIEndpoint) -OG $($WSOOGID)"'
Start-Process C:\windows\system32\WindowsPowerShell\v1.0\powershell.exe -ArgumentList $args
