...
##########################################################################################
# Name: AAD-Intune_to_WSO_Migration.ps1
# Version: 0.1
# Date: 1.11.2020
...
# - Install PPKG to renroll the device to Azure AD
...
#Download PPKG
$WebClient = New-Object System.Net.WebClient
$WebClient.DownloadFile("https://winsettings.blob.core.windows.net/winsettings/WSOMigration/PPKG_AADJoin.zip?sp=rl&st=2020-10-27T09:42:32Z&se=2029-10-21T09:42:00Z&sv=2019-12-12&sr=b&sig=7TkDrOnlMqTFqafFpxmUil8%2FM5Vw5Y0lWfW%2FCsNzL6I%3D","C:\temp\AADEnroll.zip")

#Remove if you don't ZIP the PPKG
Expand-Archive -Path "C:\Temp\AADEnroll.zip" -DestinationPath C:\Temp\AADEnroll -Force
...
#Create local admin - as Backup user and installer for the scheduled task
net user /add Installer P4ssw0rd!
...
Install-ProvisioningPackage -PackagePath 'C:\Temp\AADEnroll\AAD WSO Migration.ppkg' -ForceInstall -QuietInstall 

#Wait to finishe the PPKG installation
Start-Sleep 60

#Restart is required to rejoin the device to the domain
