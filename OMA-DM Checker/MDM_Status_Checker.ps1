...
function Print-FinalSummary {
    param (
        ...
        [switch]$SaveToRegistry, # Switch to save the summary to registry
        [string]$RegistryPath = "HKLM:\Software\WorkspaceONEStatus",
    ...
    if ($global:outputResults.Count -gt 0) {
        ...
        elseif ($SaveToRegistry) {
            ...
            foreach ($result in $global:Results) {
                ...
                # Print the result
                Write-Output "$category - $($test): $resultStatus - $details"

                # Write the result to the registry
                $categoryPath = Join-Path -Path $RegistryPath -ChildPath $category
                if (-not (Test-Path $categoryPath)) {
                    New-Item -Path $categoryPath -Force | Out-Null
                }

                Set-ItemProperty -Path $categoryPath -Name $test -Value "$resultStatus - $details" -Type String
            }
        }
        else {
